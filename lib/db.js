...
async function initSchema() {
  ...
  // El correo con el que se confirmo el registro. NULL en los usuarios que crea
  // el dueno a mano: a esos no se les pide correo, los da de alta el.
  // ALTER idempotente (falla y se ignora si la columna ya existe).
  try { await db.execute(`ALTER TABLE users ADD COLUMN email TEXT`); } catch { /* ya existe */ }
  ...
  // Un registro a medias: los datos ya validados y el codigo que se mando por
  // correo. La cuenta NO existe hasta que el codigo se confirma, asi que un
  // registro abandonado no deja usuario ni cuenta a medio hacer.
  //
  // La clave es el correo: pedir el codigo otra vez reemplaza el anterior en vez
  // de acumular filas.
  await db.execute(`
  ...
  // registro abandonado no deja usuario ni cuenta a medio hacer.
  //
  // La clave es el correo: pedir el codigo otra vez reemplaza el anterior en
  // vez de acumular filas.
  await db.execute(`
    CREATE TABLE IF NOT EXISTS signups (
      email        TEXT PRIMARY KEY,
      name         TEXT NOT NULL,
