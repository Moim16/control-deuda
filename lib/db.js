...
async function initSchema() {
  ...
  // ------------------------------------------------------------- servicios
  //
  // Un registro por visita al taller, no por cosa hecha: en la casa comercial
  // le hacen el mantenimiento completo y se paga UN monto. Ese registro marca
  // de una vez TODAS las tareas que cubrio (aceite, filtro, cadena...), y de
  // ahi sale cuando toca cada una otra vez.
  //
  // `kind`:
  //   'service'   -> mantenimiento o reparacion: puede cubrir tareas.
  //   'accessory' -> algo que se le puso (antivuelco, pescantes, llantas).
  //                  No cubre tareas: no es algo que haya que repetir, es una
  //                  mejora, y se lleva su cuenta aparte.
  ...
  await db.execute(`
    CREATE TABLE IF NOT EXISTS services (
      id         INTEGER PRIMARY KEY AUTOINCREMENT,
      vehicleId  INTEGER NOT NULL REFERENCES vehicles(id) ON DELETE CASCADE,
      taskId     INTEGER,                          -- legado: ver service_tasks
  ...
  try { await db.execute(`ALTER TABLE services ADD COLUMN kind TEXT NOT NULL DEFAULT 'service'`); } catch { /* ya existe */ }
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_srv_vehicle ON services (vehicleId, day)`);

  // Que tareas cubrio cada servicio. Antes era una sola columna `taskId` en
  // services; ahora son varias, porque un mantenimiento cubre varias cosas.
  await db.execute(`
    CREATE TABLE IF NOT EXISTS service_tasks (
      serviceId INTEGER NOT NULL REFERENCES services(id) ON DELETE CASCADE,
      taskId    INTEGER NOT NULL REFERENCES vehicle_tasks(id) ON DELETE CASCADE,
      PRIMARY KEY (serviceId, taskId)
    )`);
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_srvtask_task ON service_tasks (taskId)`);
  // Migracion de una sola vez: lo que estaba en la columna pasa a la tabla, y
  // la columna se vacia. Si no se vaciara, al desmarcar una tarea la migracion
  // la volveria a poner en el siguiente arranque.
  await db.execute(`INSERT OR IGNORE INTO service_tasks (serviceId, taskId)
                      SELECT id, taskId FROM services WHERE taskId IS NOT NULL`);
  await db.execute(`UPDATE services SET taskId = NULL WHERE taskId IS NOT NULL`);
...
export const SERVICE_KINDS = ["service", "accessory"];
