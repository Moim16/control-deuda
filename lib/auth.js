...
/* ---------------------------------------------------------------------------
   ...
   Tres cercos, uno dentro del otro:
     ...
     3. DIRECCION: los COBROS (direction='owed', lo que me deben) son solo del
        dueño. A quien entra de solo lectura se le comparte su propia deuda,
        no la contabilidad de a quien mas le presto uno: aunque por error se le
        asigne un cobro, aqui no lo ve.
...
export function debtScope(user, alias = "d") {
  ...
  return {
    sql: `${alias}.accountId = ? AND ${alias}.direction = 'owe' AND EXISTS (
